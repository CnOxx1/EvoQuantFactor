...
class DSLFactor(Factor):
    def __init__(self):
        self.spec = FactorSpec(
            ...
            lookback=20,
            horizon=5,
            params={"expression": """ma(div(abs(ret_1d),amount),5)"""},
            tags=["dsl", "loop"],
            hypothesis="""流动性/成交；模板启发表达式 ma(div(abs(ret_1d),amount),5)""",
            entry_gate="research",
            expression="""ma(div(abs(ret_1d),amount),5)""",
            mechanism="liquidity",
        )
