...
class DSLFactor(Factor):
    def __init__(self):
        self.spec = FactorSpec(
            ...
            lookback=20,
            horizon=5,
            params={"expression": """sub(rank(roc(close_adj,5)),rank(roc(vol,5)))"""},
            tags=["dsl", "loop"],
            hypothesis="""量价配合；模板启发表达式 sub(rank(roc(close_adj,5)),rank(roc(vol,5)))""",
            entry_gate="research",
            expression="""sub(rank(roc(close_adj,5)),rank(roc(vol,5)))""",
