...
class DSLFactor(Factor):
    def __init__(self):
        self.spec = FactorSpec(
            name="volume_price_pert_173855_6473",
            version="0.1.0",
            status="draft",
            family="price_volume",
            ...
            lookback=20,
            horizon=5,
            params={"expression": """sub(rank(roc(close_adj,10)),rank(roc(vol,60)))"""},
            tags=["dsl", "loop"],
            hypothesis="""量价配合；模板启发表达式 sub(rank(roc(close_adj,60)),rank(roc(vol,60))) | window perturb""",
            entry_gate="research",
            expression="""sub(rank(roc(close_adj,10)),rank(roc(vol,60)))""",
