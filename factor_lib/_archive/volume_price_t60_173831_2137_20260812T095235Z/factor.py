...
class DSLFactor(Factor):
    def __init__(self):
        self.spec = FactorSpec(
            name="volume_price_t60_173831_2137",
            version="0.1.0",
            status="draft",
            family="price_volume",
            ...
            lookback=20,
            horizon=5,
            params={"expression": """mul(rank(roc(close_adj,60)),rank(roc(vol,60)))"""},
            tags=["dsl", "loop"],
            hypothesis="""量价配合；模板启发表达式 mul(rank(roc(close_adj,60)),rank(roc(vol,60)))""",
            entry_gate="research",
            expression="""mul(rank(roc(close_adj,60)),rank(roc(vol,60)))""",
