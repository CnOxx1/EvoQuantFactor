...
class DSLFactor(Factor):
    def __init__(self):
        self.spec = FactorSpec(
            ...
            lookback=20,
            horizon=5,
            params={"expression": """mul(rank(roc(close_adj,40)),rank(roc(vol,40)))"""},
            tags=["dsl", "loop"],
            hypothesis="""量价配合；模板启发表达式 mul(rank(roc(close_adj,40)),rank(roc(vol,40)))""",
            entry_gate="research",
            expression="""mul(rank(roc(close_adj,40)),rank(roc(vol,40)))""",
