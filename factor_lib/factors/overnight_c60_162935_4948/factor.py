...
class DSLFactor(Factor):
    def __init__(self):
        self.spec = FactorSpec(
            name="overnight_c60_162935_4948",
            version="0.1.0",
            status="draft",
            family="price_volume",
            category="overnight",
            ...
            lookback=20,
            horizon=5,
            params={"expression": """div(std(overnight,60),std(amplitude,60))"""},
            tags=["dsl", "loop"],
            hypothesis="""跳空/隔夜；compose div(std(overnight,60),std(amplitude,60))""",
            entry_gate="research",
            expression="""div(std(overnight,60),std(amplitude,60))""",
            mechanism="overnight",
        )
