...
class DSLFactor(Factor):
    def __init__(self):
        self.spec = FactorSpec(
            name="volume_price_c10_142434_8634",
            version="0.1.0",
            status="draft",
            family="price_volume",
            ...
            lookback=20,
            horizon=5,
            params={"expression": """mul(rank(roc(close_adj,10)),rank(roc(amount,10)))"""},
            tags=["dsl", "loop"],
            hypothesis="""量价配合；compose mul(rank(roc(close_adj,10)),rank(roc(amount,10)))""",
            entry_gate="research",
            expression="""mul(rank(roc(close_adj,10)),rank(roc(amount,10)))""",
