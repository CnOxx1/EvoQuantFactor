...
class DSLFactor(Factor):
    def __init__(self):
        self.spec = FactorSpec(
            name="overnight_c60_164135_4146",
            version="0.1.0",
            status="draft",
            family="price_volume",
            category="overnight",
            ...
            lookback=20,
            horizon=5,
            params={"expression": """div(overnight,ma(overnight,60))"""},
            tags=["dsl", "loop"],
            hypothesis="""跳空/隔夜；compose div(overnight,ma(overnight,60))""",
            entry_gate="research",
            expression="""div(overnight,ma(overnight,60))""",
            mechanism="overnight",
        )
