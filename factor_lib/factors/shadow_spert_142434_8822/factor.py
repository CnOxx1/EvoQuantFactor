...
class DSLFactor(Factor):
    def __init__(self):
        self.spec = FactorSpec(
            ...
            lookback=20,
            horizon=5,
            params={"expression": """sub(std(upper_shadow,40),ma(lower_shadow,40))"""},
            tags=["dsl", "loop"],
            hypothesis="""影线结构；compose sub(ma(upper_shadow,40),ma(lower_shadow,40)) | structure perturb""",
            entry_gate="research",
            expression="""sub(std(upper_shadow,40),ma(lower_shadow,40))""",
            mechanism="shadow",
        )
