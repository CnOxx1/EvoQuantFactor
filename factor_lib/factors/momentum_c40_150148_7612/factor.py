...
class DSLFactor(Factor):
    def __init__(self):
        self.spec = FactorSpec(
            ...
            lookback=20,
            horizon=5,
            params={"expression": """sub(roc(close_adj,40),ma(roc(close_adj,40),10))"""},
            tags=["dsl", "loop"],
            hypothesis="""中期动量；compose sub(roc(close_adj,40),ma(roc(close_adj,40),10))""",
            entry_gate="research",
            expression="""sub(roc(close_adj,40),ma(roc(close_adj,40),10))""",
            mechanism="momentum",
        )
