...
class DSLFactor(Factor):
    def __init__(self):
        self.spec = FactorSpec(
            name="volume_price_c40_112017_9445",
            version="0.1.0",
            status="draft",
            family="price_volume",
            ...
            lookback=20,
            horizon=5,
            params={"expression": """div(rank(roc(ret_1d,40)),rank(roc(amount,40)))"""},
            tags=["dsl", "loop"],
            hypothesis="""量价配合；compose div(rank(roc(ret_1d,40)),rank(roc(amount,40)))""",
            entry_gate="research",
            expression="""div(rank(roc(ret_1d,40)),rank(roc(amount,40)))""",
