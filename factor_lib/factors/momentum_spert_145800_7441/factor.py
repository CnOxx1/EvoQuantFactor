...
class DSLFactor(Factor):
    def __init__(self):
        self.spec = FactorSpec(
            name="momentum_spert_145800_7441",
            version="0.1.0",
            status="draft",
            family="price_volume",
            category="momentum",
            ...
            lookback=20,
            horizon=5,
            params={"expression": """rank(sub(roc(close_adj,60),ma(roc(close_adj,60),5)))"""},
            tags=["dsl", "loop"],
            hypothesis="""中期动量；compose sub(roc(close_adj,60),ma(roc(close_adj,60),5)) | structure perturb""",
            entry_gate="research",
            expression="""rank(sub(roc(close_adj,60),ma(roc(close_adj,60),5)))""",
            mechanism="momentum",
        )
