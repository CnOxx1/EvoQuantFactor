...
class DSLFactor(Factor):
    def __init__(self):
        self.spec = FactorSpec(
            ...
            lookback=20,
            horizon=5,
            params={"expression": """std(roc(upper_shadow,60),5)"""},
            tags=["dsl", "loop"],
            hypothesis="""影线结构；compose std(roc(upper_shadow,60),5)""",
            entry_gate="research",
            expression="""std(roc(upper_shadow,60),5)""",
            mechanism="shadow",
        )
