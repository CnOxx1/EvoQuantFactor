...
class DSLFactor(Factor):
    def __init__(self):
        self.spec = FactorSpec(
            name="overnight_c20_104204_1181",
            version="0.1.0",
            status="draft",
            family="price_volume",
            category="overnight",
            ...
            lookback=20,
            horizon=5,
            params={"expression": """div(std(overnight,20),ma(abs(overnight),20))"""},
            tags=["dsl", "loop"],
            hypothesis="""跳空/隔夜；compose div(std(overnight,20),ma(abs(overnight),20))""",
            entry_gate="research",
            expression="""div(std(overnight,20),ma(abs(overnight),20))""",
            mechanism="overnight",
        )
