...
class DSLFactor(Factor):
    def __init__(self):
        self.spec = FactorSpec(
            name="liquidity_c20_103651_4320",
            version="0.1.0",
            status="draft",
            family="price_volume",
            category="liquidity",
            ...
            lookback=20,
            horizon=5,
            params={"expression": """sub(ma(vol,20),ma(amount,20))"""},
            tags=["dsl", "loop"],
            hypothesis="""流动性/成交；compose sub(ma(vol,20),ma(amount,20))""",
            entry_gate="research",
            expression="""sub(ma(vol,20),ma(amount,20))""",
            mechanism="liquidity",
        )
