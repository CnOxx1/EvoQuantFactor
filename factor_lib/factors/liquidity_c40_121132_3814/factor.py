...
class DSLFactor(Factor):
    def __init__(self):
        self.spec = FactorSpec(
            name="liquidity_c40_121132_3814",
            version="0.1.0",
            status="draft",
            family="price_volume",
            category="liquidity",
            ...
            lookback=20,
            horizon=5,
            params={"expression": """delta(ma(turnover_rate,40),10)"""},
            tags=["dsl", "loop"],
            hypothesis="""流动性/成交；compose delta(ma(turnover_rate,40),10)""",
            entry_gate="research",
            expression="""delta(ma(turnover_rate,40),10)""",
            mechanism="liquidity",
        )
