...
class DSLFactor(Factor):
    def __init__(self):
        self.spec = FactorSpec(
            name="liquidity_c20_142949_2677",
            version="0.1.0",
            status="draft",
            family="price_volume",
            category="liquidity",
            ...
            lookback=20,
            horizon=5,
            params={"expression": """div(roc(amount,20),std(ret_1d,20))"""},
            tags=["dsl", "loop"],
            hypothesis="""流动性/成交；compose div(roc(amount,20),std(ret_1d,20))""",
            entry_gate="research",
            expression="""div(roc(amount,20),std(ret_1d,20))""",
            mechanism="liquidity",
        )
