...
class DSLFactor(Factor):
    def __init__(self):
        self.spec = FactorSpec(
            name="volume_price_c60_104125_1070",
            version="0.1.0",
            status="draft",
            family="price_volume",
            ...
            lookback=20,
            horizon=5,
            params={"expression": """div(rank(roc(close_adj,60)),rank(roc(amount,60)))"""},
            tags=["dsl", "loop"],
            hypothesis="""量价配合；compose div(rank(roc(close_adj,60)),rank(roc(amount,60)))""",
            entry_gate="research",
            expression="""div(rank(roc(close_adj,60)),rank(roc(amount,60)))""",
