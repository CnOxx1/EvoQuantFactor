...
class DSLFactor(Factor):
    def __init__(self):
        self.spec = FactorSpec(
            name="amplitude_c40_120226_4503",
            version="0.1.0",
            status="draft",
            family="price_volume",
            category="amplitude",
            ...
            lookback=20,
            horizon=5,
            params={"expression": """delta(ma(amplitude,40),10)"""},
            tags=["dsl", "loop"],
            hypothesis="""振幅结构；compose delta(ma(amplitude,40),10)""",
            entry_gate="research",
            expression="""delta(ma(amplitude,40),10)""",
            mechanism="amplitude",
        )
