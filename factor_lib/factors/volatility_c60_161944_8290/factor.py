...
class DSLFactor(Factor):
    def __init__(self):
        self.spec = FactorSpec(
            name="volatility_c60_161944_8290",
            version="0.1.0",
            status="draft",
            family="price_volume",
            category="volatility",
            ...
            lookback=20,
            horizon=5,
            params={"expression": """div(std(ret_1d,60),std(overnight,60))"""},
            tags=["dsl", "loop"],
            hypothesis="""波动率；compose div(std(ret_1d,60),std(overnight,60))""",
            entry_gate="research",
            expression="""div(std(ret_1d,60),std(overnight,60))""",
