...
class DSLFactor(Factor):
    def __init__(self):
        self.spec = FactorSpec(
            ...
            lookback=20,
            horizon=5,
            params={"expression": """delta(neg(roc(close_adj,5)),20)"""},
            tags=["dsl", "loop"],
            hypothesis="""短期反转；compose delta(neg(roc(close_adj,5)),20)""",
            entry_gate="research",
            expression="""delta(neg(roc(close_adj,5)),20)""",
            mechanism="reversal",
        )
