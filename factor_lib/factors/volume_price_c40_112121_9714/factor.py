...
class DSLFactor(Factor):
    def __init__(self):
        self.spec = FactorSpec(
            name="volume_price_c40_112121_9714",
            version="0.1.0",
            status="draft",
            family="price_volume",
            ...
            lookback=20,
            horizon=5,
            params={"expression": """sub(rank(delta(open,40)),rank(roc(vol,40)))"""},
            tags=["dsl", "loop"],
            hypothesis="""量价配合；compose sub(rank(delta(open,40)),rank(roc(vol,40)))""",
            entry_gate="research",
            expression="""sub(rank(delta(open,40)),rank(roc(vol,40)))""",
