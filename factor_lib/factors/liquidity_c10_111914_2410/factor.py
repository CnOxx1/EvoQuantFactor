...
class DSLFactor(Factor):
    def __init__(self):
        self.spec = FactorSpec(
            name="liquidity_c10_111914_2410",
            version="0.1.0",
            status="draft",
            family="price_volume",
            category="liquidity",
            ...
            lookback=20,
            horizon=5,
            params={"expression": """delay(roc(turnover_rate,10),20)"""},
            tags=["dsl", "loop"],
            hypothesis="""流动性/成交；compose delay(roc(turnover_rate,10),20)""",
            entry_gate="research",
            expression="""delay(roc(turnover_rate,10),20)""",
            mechanism="liquidity",
        )
