...
class DSLFactor(Factor):
    def __init__(self):
        self.spec = FactorSpec(
            ...
            lookback=20,
            horizon=5,
            params={"expression": """sub(rank(upper_shadow),rank(roc(close_adj,5)))"""},
            tags=["dsl", "loop"],
            hypothesis="""影线结构；compose sub(rank(upper_shadow),rank(roc(close_adj,5)))""",
            entry_gate="research",
            expression="""sub(rank(upper_shadow),rank(roc(close_adj,5)))""",
            mechanism="shadow",
        )
