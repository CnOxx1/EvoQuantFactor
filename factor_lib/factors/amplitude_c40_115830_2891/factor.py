...
class DSLFactor(Factor):
    def __init__(self):
        self.spec = FactorSpec(
            name="amplitude_c40_115830_2891",
            version="0.1.0",
            status="draft",
            family="price_volume",
            category="amplitude",
            ...
            lookback=20,
            horizon=5,
            params={"expression": """div(std(amplitude,40),ma(abs(amplitude),40))"""},
            tags=["dsl", "loop"],
            hypothesis="""振幅结构；compose div(std(amplitude,40),ma(abs(amplitude),40))""",
            entry_gate="research",
            expression="""div(std(amplitude,40),ma(abs(amplitude),40))""",
            mechanism="amplitude",
        )
