...
class DSLFactor(Factor):
    def __init__(self):
        self.spec = FactorSpec(
            name="amplitude_c10_105336_3673",
            version="0.1.0",
            status="draft",
            family="price_volume",
            category="amplitude",
            ...
            lookback=20,
            horizon=5,
            params={"expression": """div(std(low,10),ma(abs(low),10))"""},
            tags=["dsl", "loop"],
            hypothesis="""振幅结构；compose div(std(low,10),ma(abs(low),10))""",
            entry_gate="research",
            expression="""div(std(low,10),ma(abs(low),10))""",
            mechanism="amplitude",
        )
