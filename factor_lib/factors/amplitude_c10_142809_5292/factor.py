...
class DSLFactor(Factor):
    def __init__(self):
        self.spec = FactorSpec(
            name="amplitude_c10_142809_5292",
            version="0.1.0",
            status="draft",
            family="price_volume",
            category="amplitude",
            ...
            lookback=20,
            horizon=5,
            params={"expression": """rank(delta(amplitude,10))"""},
            tags=["dsl", "loop"],
            hypothesis="""振幅结构；compose rank(delta(amplitude,10))""",
            entry_gate="research",
            expression="""rank(delta(amplitude,10))""",
            mechanism="amplitude",
        )
