...
class DSLFactor(Factor):
    def __init__(self):
        self.spec = FactorSpec(
            name="liquidity_c10_110935_9867",
            version="0.1.0",
            status="draft",
            family="price_volume",
            category="liquidity",
            ...
            lookback=20,
            horizon=5,
            params={"expression": """neg(roc(turnover_rate,10))"""},
            tags=["dsl", "loop"],
            hypothesis="""流动性/成交；compose neg(roc(turnover_rate,10))""",
            entry_gate="research",
            expression="""neg(roc(turnover_rate,10))""",
            mechanism="liquidity",
        )
