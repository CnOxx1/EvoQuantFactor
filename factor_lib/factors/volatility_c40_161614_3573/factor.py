...
class DSLFactor(Factor):
    def __init__(self):
        self.spec = FactorSpec(
            name="volatility_c40_161614_3573",
            version="0.1.0",
            status="draft",
            family="price_volume",
            category="volatility",
            ...
            lookback=20,
            horizon=5,
            params={"expression": """div(std(ret_1d,40),std(overnight,40))"""},
            tags=["dsl", "loop"],
            hypothesis="""波动率；compose div(std(ret_1d,40),std(overnight,40))""",
            entry_gate="research",
            expression="""div(std(ret_1d,40),std(overnight,40))""",
