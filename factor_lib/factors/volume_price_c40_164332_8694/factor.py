...
class DSLFactor(Factor):
    def __init__(self):
        self.spec = FactorSpec(
            name="volume_price_c40_164332_8694",
            version="0.1.0",
            status="draft",
            family="price_volume",
            ...
            lookback=20,
            horizon=5,
            params={"expression": """div(ma(close_adj,40),ma(vol,40))"""},
            tags=["dsl", "loop"],
            hypothesis="""量价配合；compose div(ma(close_adj,40),ma(vol,40))""",
            entry_gate="research",
            expression="""div(ma(close_adj,40),ma(vol,40))""",
