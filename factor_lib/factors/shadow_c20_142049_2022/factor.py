...
class DSLFactor(Factor):
    def __init__(self):
        self.spec = FactorSpec(
            ...
            lookback=20,
            horizon=5,
            params={"expression": """sub(ma(upper_shadow,20),ma(lower_shadow,20))"""},
            tags=["dsl", "loop"],
            hypothesis="""影线结构；compose sub(ma(upper_shadow,20),ma(lower_shadow,20))""",
            entry_gate="research",
            expression="""sub(ma(upper_shadow,20),ma(lower_shadow,20))""",
            mechanism="shadow",
        )
