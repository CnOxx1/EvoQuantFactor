...
class DSLFactor(Factor):
    def __init__(self):
        self.spec = FactorSpec(
            name="volume_price_xover_145027_6282",
            version="0.1.0",
            status="draft",
            family="price_volume",
            ...
            lookback=20,
            horizon=5,
            params={"expression": """sub(rank(delta(ret_1d, 60)), ma(turnover_rate, 60))"""},
            tags=["dsl", "loop"],
            hypothesis="""crossover volatility x liquidity""",
            entry_gate="research",
            expression="""sub(rank(delta(ret_1d, 60)), ma(turnover_rate, 60))""",
