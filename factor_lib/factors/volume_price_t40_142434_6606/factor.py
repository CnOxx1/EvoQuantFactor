...
class DSLFactor(Factor):
    def __init__(self):
        self.spec = FactorSpec(
            name="volume_price_t40_142434_6606",
            version="0.1.0",
            status="draft",
            family="price_volume",
            ...
            lookback=20,
            horizon=5,
            params={"expression": """sub(rank(delta(close_adj,40)),rank(roc(amount,40)))"""},
            tags=["dsl", "loop"],
            hypothesis="""量价配合；模板启发表达式 sub(rank(delta(close_adj,40)),rank(roc(amount,40)))""",
            entry_gate="research",
            expression="""sub(rank(delta(close_adj,40)),rank(roc(amount,40)))""",
