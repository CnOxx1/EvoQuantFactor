...
class DSLFactor(Factor):
    def __init__(self):
        self.spec = FactorSpec(
            name="amplitude_c40_171144_1084",
            version="0.1.0",
            status="draft",
            family="price_volume",
            category="amplitude",
            ...
            lookback=20,
            horizon=5,
            params={"expression": """delay(roc(amplitude,40),60)"""},
            tags=["dsl", "loop"],
            hypothesis="""振幅结构；compose delay(roc(amplitude,40),60)""",
            entry_gate="research",
            expression="""delay(roc(amplitude,40),60)""",
            mechanism="amplitude",
        )
