...
class DSLFactor(Factor):
    def __init__(self):
        self.spec = FactorSpec(
            name="volume_price_t20_173839_5333",
            version="0.1.0",
            status="draft",
            family="price_volume",
            ...
            lookback=20,
            horizon=5,
            params={"expression": """sub(rank(roc(close_adj,20)),rank(roc(vol,20)))"""},
            tags=["dsl", "loop"],
            hypothesis="""量价配合；模板启发表达式 sub(rank(roc(close_adj,20)),rank(roc(vol,20)))""",
            entry_gate="research",
            expression="""sub(rank(roc(close_adj,20)),rank(roc(vol,20)))""",
