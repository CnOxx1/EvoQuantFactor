...
class DSLFactor(Factor):
    def __init__(self):
        self.spec = FactorSpec(
            ...
            lookback=20,
            horizon=5,
            params={"expression": """div(ma(upper_shadow,5),ma(lower_shadow,5))"""},
            tags=["dsl", "loop"],
            hypothesis="""影线结构；compose div(ma(upper_shadow,5),ma(lower_shadow,5))""",
            entry_gate="research",
            expression="""div(ma(upper_shadow,5),ma(lower_shadow,5))""",
            mechanism="shadow",
        )
