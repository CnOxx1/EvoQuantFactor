...
class DSLFactor(Factor):
    def __init__(self):
        self.spec = FactorSpec(
            name="amplitude_c20_115933_1120",
            version="0.1.0",
            status="draft",
            family="price_volume",
            category="amplitude",
            ...
            lookback=20,
            horizon=5,
            params={"expression": """div(std(low,20),ma(abs(low),20))"""},
            tags=["dsl", "loop"],
            hypothesis="""振幅结构；compose div(std(low,20),ma(abs(low),20))""",
            entry_gate="research",
            expression="""div(std(low,20),ma(abs(low),20))""",
            mechanism="amplitude",
        )
