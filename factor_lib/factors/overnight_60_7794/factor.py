...
class DSLFactor(Factor):
    def __init__(self):
        self.spec = FactorSpec(
            ...
            lookback=20,
            horizon=5,
            params={"expression": """sub(ma(overnight,60),ma(amplitude,60))"""},
            tags=["dsl", "loop"],
            hypothesis="""跳空/隔夜；模板启发表达式 sub(ma(overnight,60),ma(amplitude,60))""",
            entry_gate="research",
            expression="""sub(ma(overnight,60),ma(amplitude,60))""",
            mechanism="overnight",
        )
