...
class DSLFactor(Factor):
    def __init__(self):
        self.spec = FactorSpec(
            name="volume_price_xover_150121_4409",
            version="0.1.0",
            status="draft",
            family="price_volume",
            ...
            lookback=20,
            horizon=5,
            params={"expression": """sub(div(std(overnight, 40), std(amplitude, 40)), rank(roc(overnight, 60)))"""},
            tags=["dsl", "loop"],
            hypothesis="""crossover volatility x overnight""",
            entry_gate="research",
            expression="""sub(div(std(overnight, 40), std(amplitude, 40)), rank(roc(overnight, 60)))""",
