...
class DSLFactor(Factor):
    def __init__(self):
        self.spec = FactorSpec(
            ...
            lookback=20,
            horizon=5,
            params={"expression": """div(std(upper_shadow,5),ma(abs(upper_shadow),5))"""},
            tags=["dsl", "loop"],
            hypothesis="""影线结构；compose div(std(upper_shadow,5),ma(abs(upper_shadow),5))""",
            entry_gate="research",
            expression="""div(std(upper_shadow,5),ma(abs(upper_shadow),5))""",
            mechanism="shadow",
        )
