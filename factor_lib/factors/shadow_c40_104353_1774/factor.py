...
class DSLFactor(Factor):
    def __init__(self):
        self.spec = FactorSpec(
            ...
            lookback=20,
            horizon=5,
            params={"expression": """rank(roc(upper_shadow,40))"""},
            tags=["dsl", "loop"],
            hypothesis="""影线结构；compose rank(roc(upper_shadow,40))""",
            entry_gate="research",
            expression="""rank(roc(upper_shadow,40))""",
            mechanism="shadow",
        )
