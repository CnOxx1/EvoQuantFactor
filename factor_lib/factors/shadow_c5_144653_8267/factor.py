...
class DSLFactor(Factor):
    def __init__(self):
        self.spec = FactorSpec(
            ...
            lookback=20,
            horizon=5,
            params={"expression": """mul(rank(turnover_rate),ma(upper_shadow,5))"""},
            tags=["dsl", "loop"],
            hypothesis="""影线结构；compose mul(rank(turnover_rate),ma(upper_shadow,5))""",
            entry_gate="research",
            expression="""mul(rank(turnover_rate),ma(upper_shadow,5))""",
            mechanism="shadow",
        )
