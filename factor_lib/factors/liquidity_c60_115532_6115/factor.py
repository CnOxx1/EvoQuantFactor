...
class DSLFactor(Factor):
    def __init__(self):
        self.spec = FactorSpec(
            name="liquidity_c60_115532_6115",
            version="0.1.0",
            status="draft",
            family="price_volume",
            category="liquidity",
            ...
            lookback=20,
            horizon=5,
            params={"expression": """div(ma(vol,60),ma(amount,60))"""},
            tags=["dsl", "loop"],
            hypothesis="""流动性/成交；compose div(ma(vol,60),ma(amount,60))""",
            entry_gate="research",
            expression="""div(ma(vol,60),ma(amount,60))""",
            mechanism="liquidity",
        )
