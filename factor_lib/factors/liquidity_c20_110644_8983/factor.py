...
class DSLFactor(Factor):
    def __init__(self):
        self.spec = FactorSpec(
            name="liquidity_c20_110644_8983",
            version="0.1.0",
            status="draft",
            family="price_volume",
            category="liquidity",
            ...
            lookback=20,
            horizon=5,
            params={"expression": """delay(roc(turnover_rate,20),10)"""},
            tags=["dsl", "loop"],
            hypothesis="""流动性/成交；compose delay(roc(turnover_rate,20),10)""",
            entry_gate="research",
            expression="""delay(roc(turnover_rate,20),10)""",
            mechanism="liquidity",
        )
