...
class DSLFactor(Factor):
    def __init__(self):
        self.spec = FactorSpec(
            name="volume_price_t20_114417_5194",
            version="0.1.0",
            status="draft",
            family="price_volume",
            ...
            lookback=20,
            horizon=5,
            params={"expression": """mul(rank(roc(close_adj,20)),rank(roc(vol,20)))"""},
            tags=["dsl", "loop"],
            hypothesis="""量价配合；模板启发表达式 mul(rank(roc(close_adj,20)),rank(roc(vol,20)))""",
            entry_gate="research",
            expression="""mul(rank(roc(close_adj,20)),rank(roc(vol,20)))""",
