...
class DSLFactor(Factor):
    def __init__(self):
        self.spec = FactorSpec(
            name="liquidity_c10_123110_2470",
            version="0.1.0",
            status="draft",
            family="price_volume",
            category="liquidity",
            ...
            lookback=20,
            horizon=5,
            params={"expression": """std(roc(turnover_rate,10),60)"""},
            tags=["dsl", "loop"],
            hypothesis="""流动性/成交；compose std(roc(turnover_rate,10),60)""",
            entry_gate="research",
            expression="""std(roc(turnover_rate,10),60)""",
            mechanism="liquidity",
        )
