...
class DSLFactor(Factor):
    def __init__(self):
        self.spec = FactorSpec(
            name="volume_price_c60_170245_3875",
            version="0.1.0",
            status="draft",
            family="price_volume",
            ...
            lookback=20,
            horizon=5,
            params={"expression": """div(ma(close_adj,60),ma(turnover_rate,60))"""},
            tags=["dsl", "loop"],
            hypothesis="""量价配合；compose div(ma(close_adj,60),ma(turnover_rate,60))""",
            entry_gate="research",
            expression="""div(ma(close_adj,60),ma(turnover_rate,60))""",
