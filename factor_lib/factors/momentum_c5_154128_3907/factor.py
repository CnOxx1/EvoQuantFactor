...
class DSLFactor(Factor):
    def __init__(self):
        self.spec = FactorSpec(
            ...
            lookback=20,
            horizon=5,
            params={"expression": """div(delta(close_adj,5), std(close_adj,40))"""},
            tags=["dsl", "loop"],
            hypothesis="""中期动量；compose div(delta(close_adj,5), std(close_adj,40))""",
            entry_gate="research",
            expression="""div(delta(close_adj,5), std(close_adj,40))""",
            mechanism="momentum",
        )
