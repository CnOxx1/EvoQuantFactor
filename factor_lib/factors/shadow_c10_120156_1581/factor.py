...
class DSLFactor(Factor):
    def __init__(self):
        self.spec = FactorSpec(
            ...
            lookback=20,
            horizon=5,
            params={"expression": """delta(ma(lower_shadow,10),60)"""},
            tags=["dsl", "loop"],
            hypothesis="""影线结构；compose delta(ma(lower_shadow,10),60)""",
            entry_gate="research",
            expression="""delta(ma(lower_shadow,10),60)""",
            mechanism="shadow",
        )
