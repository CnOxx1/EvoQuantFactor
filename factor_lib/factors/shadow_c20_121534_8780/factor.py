...
class DSLFactor(Factor):
    def __init__(self):
        self.spec = FactorSpec(
            ...
            lookback=20,
            horizon=5,
            params={"expression": """ma(roc(upper_shadow,20),5)"""},
            tags=["dsl", "loop"],
            hypothesis="""影线结构；compose ma(roc(upper_shadow,20),5)""",
            entry_gate="research",
            expression="""ma(roc(upper_shadow,20),5)""",
            mechanism="shadow",
        )
