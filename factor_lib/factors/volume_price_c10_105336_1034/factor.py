...
class DSLFactor(Factor):
    def __init__(self):
        self.spec = FactorSpec(
            name="volume_price_c10_105336_1034",
            version="0.1.0",
            status="draft",
            family="price_volume",
            ...
            lookback=20,
            horizon=5,
            params={"expression": """sub(rank(roc(ret_1d,10)),rank(roc(amount,10)))"""},
            tags=["dsl", "loop"],
            hypothesis="""量价配合；compose sub(rank(roc(ret_1d,10)),rank(roc(amount,10)))""",
            entry_gate="research",
            expression="""sub(rank(roc(ret_1d,10)),rank(roc(amount,10)))""",
