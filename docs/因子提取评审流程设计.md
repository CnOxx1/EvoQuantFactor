...
> **边界**：本文描述的是 **因子层（单因子质量门禁）**。多因子组成策略属于独立的策略层，不改写本文门禁与六角色职责；见 [归档 · 多因子策略层设计](./archive/多因子策略层设计.md)、[更新记录](./更新记录.md)。
...
| 拆分 | `BOOTSTRAP_PROFILE=split ./scripts/bootstrap.sh` | API + 独立 worker/collector |

原则：

- 密钥只放 `.env`（不进镜像、不进 Git）
- 提示词 `prompts/`、业务配置 `config/` 挂载进容器，改完重启即可
- Step3 门槛在应用内用代码计算（`factor_backend/services/router_logic.py`）
- 行情 MCP 未就绪时可只起 `factor-api`；就绪后改 `MCP_ENABLED` 与镜像即可

完整说明见：[一键部署与打包](./一键部署与打包.md)、根目录 [README.md](../README.md)。

早期方案稿（已归档，部分内容过时）见：[开发方案：LangGraph + Docker](./archive/开发方案-LangGraph-Docker.md)。
