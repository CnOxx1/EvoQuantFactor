...
量化实践里常见两条链路，**本系统当前只覆盖因子层**：

```text
因子层（已实现）          策略层（规划，见 docs/archive/多因子策略层设计.md）
研报/库 → 单因子评审门禁 → 入库     →  多因子勾选合成 → 策略级评估 → 策略库
```

| 维度 | 因子层（本文 / 现网） | 策略层（后续） |
|------|----------------------|----------------|
| 目标 | 单因子可算、逻辑、稳健、否决 | 组合冗余、权重、合成可解释性；（真回测另接） |
| 输入 | 1 条公式定义 | N 条已入库因子 + 合成规则 |
| 失败语义 | 该因子 REVISE/DROP | 策略不通过；成分因子仍可留在任务入库 |
| 门禁 | 现有 `decide_action` | **独立**规则与提示词，不改写单因子门禁 |

**结论：不需要把现有流程改成「一次只评一整套策略」。** 别人说的多因子策略，是在因子过筛选之后的组合层。

相关变更记录：[更新记录.md](./更新记录.md)。

## 1. 背景与目标

系统原有主链路是：**研报文本 → LLM 提取因子 → 六角色评审 → 门禁裁决 → 多轮修订 → 落盘**。

「因子优化」在此基础上增加一条旁路：用户从**因子库**（Alpha101 / 任务入库 / 淘汰库）勾选已有因子，**跳过研报提取**，将公式直接作为种子注入同一套 LLM 评估与修订流水线，用于：

- 对公开公式（如 Alpha101）做可实现性 / 稳健性再评估；
- 对历史任务入库或淘汰库中的因子做二次优化与评分；
- 保留完整步骤时间线，便于对比修订前后。

> 注意：即使一次勾选多个因子创建 `mode=evaluate` 任务，流水线仍是**逐因子**评审与门禁，不是策略组合评估。
...
| Tab | pack_id | 数据 | 说明 |
|-----|---------|------|------|
| **Alpha101** | `alpha101` | `data/factor_library/alpha101.json` | 公开公式，只读 |
| **任务入库** | `workspace` | `data/factor_library/workspace.json` | 门禁 SAVE 落库 |
| **淘汰库** | `dropped` | `data/factor_library/dropped.json` | 门禁 DROP 落库（含淘汰原因） |

读取：`GET /api/v1/factor-library/{pack_id}/factors`（分页 / 搜索）。
...
| 方法 | 路径 | 说明 |
|------|------|------|
| GET | `/api/v1/factor-library/packs` | 包列表：`alpha101` / `workspace` / `dropped` |
| GET | `/api/v1/factor-library/{pack_id}/factors` | 分页 / 搜索（支持 `q` / `limit` / `offset`） |

数据文件：

```text
data/factor_library/
  alpha101.json     # 公开库（只读）
  workspace.json    # 任务入库 SAVE（运行时写入）
  dropped.json      # 淘汰库 DROP（运行时写入）
```

因子主键在可变库中为 `{job_id}:{origin_factor_id}`，优化提交时用 `origin_factor_id` 作为种子 ID。

### 5.3 任务因子结果

`GET /api/v1/jobs/{id}/factors?include_candidates=true`（默认 true）

- `status=SAVE`：过线因子
- `status=DROP`（历史兼容字段名 `candidates`）：淘汰但保留公式的因子

### 5.4 再次分析

`POST /api/v1/jobs/{id}/rerun`：复用原 `report_id` 与关键 meta（含 `mode` / `seed_factors`）新建任务。
...
| 未过线且 `round < max_round` | **REVISE** → 生成 `revise_packet` 回 step1（每轮最多 5 个最低分） |
| 否则 | **DROP** |

### 6.6 persist

- SAVE → 任务结果 `factors` + 写入 **任务入库** `workspace.json`
- 有公式的 DROP → 任务结果 `candidates`（status=`DROP`）+ 写入 **淘汰库** `dropped.json`
- `formula_or_rule` 从 `definition` 抽出；兼容字符串型 `definition` 与顶层 `name` 字段
- Alpha101 文件不会被改写

实现：`upsert_job_factors_to_workspace` / `upsert_job_factors_to_dropped`（`factor_library.py`）。
...
用户点击「优化因子」
  ...
  → 六角色评审（library_optimize 提示）→ 合并 → 门禁
      ├─ SAVE → persist → 任务入库 workspace.json
      ├─ REVISE → step1 + step1_optimize.json → 再评审…
      └─ DROP  → persist → 淘汰库 dropped.json
```

## 10. 配置与运维

- 默认 `max_round`、门禁阈值来自环境变量 / `Settings`（`config/default.yaml` 仅参考，不加载）
...
2. **Alpha101 写回**：优化 / 研报结果写入 `workspace` / `dropped`，**不**自动改写 `alpha101.json`。
3. **批量上限**：未限制勾选数量；大批量会放大 LLM 成本；修订侧已按每轮最多 5 个限流。
4. **跨页勾选**：Alpha101 翻页会清理不在本页的勾选。
5. **角色提示词**：目前用运行时追加；若优化评分维度需大改，可再拆 `step2_*_optimize` 专用包。

变更历史见：[docs/更新记录.md](docs/更新记录.md)。  
策略层实现计划见：[archive/多因子策略层设计.md](./archive/多因子策略层设计.md)（未改本流水线代码）。
...
| 创建任务 / stub 报告 / rerun | `backend/factor_backend/api/routes_jobs.py` |
| 种子短路 / 落盘 / 步骤记录 | `backend/factor_backend/graph/nodes.py` |
| 图编排 | `backend/factor_backend/graph/build.py` |
| 门禁规则 | `backend/factor_backend/services/router_logic.py` |
| 因子库服务（三分区） | `backend/factor_backend/services/factor_library.py` |
| 因子库页 | `frontend/src/views/FactorsView.vue` |
| Alpha101 / 入库 / 淘汰数据 | `data/factor_library/*.json` |
